...
class SynthesizerEval(nn.Module):
    ...
    def inference_stream(self, x, x_lengths, bert, sid=None, noise_scale=1, length_scale=1):
        """
        流式生成 TTS 语音，支持边生成边输出
        """
        print("-----------------------------------------------------------------------------")
        print(datetime.datetime.now())
        ...
        if self.n_speakers > 0:
            g = self.emb_g(sid).unsqueeze(-1)  # 说话人嵌入
        else:
            g = None
...
#
        ...
        # **Step 5: 流式语音合成**
        while stream_index + stream_chunk < len_z:
            cut_s = max(0, stream_index - hop_frame)
            cut_e = min(len_z, stream_index + stream_chunk + hop_frame)
            ...
            o_chunk = self.dec(z_chunk, g=g)[0, 0].data.cpu().float().numpy()
            o_chunk = o_chunk[max(0, hop_sample if stream_index > 0 else 0):-hop_sample]

            # 处理重叠部分
            if wav_gen_prev is not None:
                crossfade = np.hanning(len(wav_overlap) * 2)[:len(wav_overlap)]
                o_chunk[:len(wav_overlap)] = (o_chunk[:len(wav_overlap)] * (1 - crossfade)) + (wav_overlap * crossfade)

            wav_overlap = o_chunk[-hop_sample:]  # 保存末尾部分
            wav_gen_prev = o_chunk  # 记录上一块音频

            yield o_chunk  # 逐步返回语音片段
        ...
        if stream_index < len_z:
            ...
            z_chunk = self.flow(z_chunk, m_chunk, g=g, reverse=True)
            o_chunk = self.dec(z_chunk, g=g)[0, 0].data.cpu().float().numpy()
            o_chunk = o_chunk[max(0, hop_sample):]
            yield o_chunk  # 确保返回最后一部分音频
